  ...
  void draw() {
    ...
    if (isPlaying) {// game start
      ...
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 10; j++) {
          if (fx <= locationX[i][j] + 50 && fx > locationX[i][j] - 50) {
            if (fy <= locationY[i][j] + 50
                && fy >= locationY[i][j] - 50) {
              updateEnemy(i);
      ...
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 10; j++) {
          for (int k = 0; k < 5; k++) {
            if (bullet_appear[k]) {
              if (!bullet_crush[k]) {
                if (locationX[i][j] + 30 >= (int) bulletPositionX[k] - 15
                    && locationX[i][j] - 30 <= (int) bulletPositionX[k] + 15) {
                  if (locationY[i][j] + 30 >= (int) bulletPositionY[k] - 13
                      && locationY[i][j] - 30 <= (int) bulletPositionY[k] + 13) {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        if (changeEnemyPlace0) {
          if ((enemynum == i && enemygroup == 0)
              || (senemynum == i && senemygroup == 0)) {
